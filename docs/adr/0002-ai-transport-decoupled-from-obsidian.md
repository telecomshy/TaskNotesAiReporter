...
同一纯度原则下，`client.ts` 的超时用**全局** `setTimeout` / `clearTimeout`（而非 `window.*`），以便在 Node 环境可运行；因此插件目录扫描的 `obsidianmd/prefer-window-timers` 会报 warning，属**有意保留**（超时是后台逻辑、与 popout 无关）。
