...
const a = makeTask({ path: "a", title: "Alpha", due: "2024-03-05", tags: ["work"] });
const b = makeTask({ path: "b", title: "Beta", due: "2024-03-10", tags: ["home"], contexts: ["home"] });
const c = makeTask({ path: "c", title: "Gamma", due: "2024-04-01", tags: ["work/proj"] });
const tasks = [a, b, c];

const march: DateRange = { start: "2024-03-01", end: "2024-03-31" };

function paths(list: { path: string }[]): string[] {
	return list.map((task) => task.path);
}
...
test("初值：时间页、无区间、空查询、无勾选", () => {
	const session = createSession(tasks, dateFields);
	...
	assert.equal(session.allTasks, tasks);
	assert.equal(session.dateFields, dateFields);
});

test("switchTab：时间页全选可见，标题页默认全不选", () => {
	const timed = setRange(createSession(tasks, dateFields), march);
	...
	assert.deepEqual(paths(visibleTasks(title)), ["a", "b", "c"]);
...
test("setRange：重算可见并全选", () => {
	const session = setRange(createSession(tasks, dateFields), march);
	assert.deepEqual(paths(visibleTasks(session)), ["a", "b"]);
	assert.deepEqual(sorted(session.checked), ["a", "b"]);
});

test("setRange：空区间无可见、无勾选", () => {
	const session = setRange(createSession(tasks, dateFields), null);
	assert.deepEqual(visibleTasks(session), []);
	assert.deepEqual(sorted(session.checked), []);
});

test("setQuery：清空勾选并按标题维度过滤", () => {
	const title = switchTab(setRange(createSession(tasks, dateFields), march), "title");
	...
	assert.deepEqual(paths(visibleTasks(byKeyword)), ["a"]);

	const byTag = setQuery(title, "#work");
	assert.deepEqual(paths(visibleTasks(byTag)).sort(), ["a", "c"]);
});

test("toggle：勾选与取消单个任务，且不改变原会话", () => {
	const title = setQuery(switchTab(createSession(tasks, dateFields), "title"), "beta");
...
test("setAll：对当前可见任务整体全选 / 取消", () => {
	const title = switchTab(createSession(tasks, dateFields), "title");
...
test("clearSelection：清空当前可见任务的勾选", () => {
	const session = clearSelection(setRange(createSession(tasks, dateFields), march));
	assert.deepEqual(sorted(session.checked), []);
});

test("visibleTasks：时间页按区间过滤、标题页按解析查询过滤", () => {
	const timed = setRange(createSession(tasks, dateFields), { start: "2024-04-01", end: "2024-04-30" });
	assert.deepEqual(paths(visibleTasks(timed)), ["c"]);

	const title = switchTab(createSession(tasks, dateFields), "title");
	assert.deepEqual(paths(visibleTasks(title)), ["a", "b", "c"]);

	const byContext = setQuery(title, "@home");
	assert.deepEqual(paths(visibleTasks(byContext)), ["b"]);
});

test("visibleTasks：排除已归档任务", () => {
	const archived = makeTask({ path: "z", title: "Archived", due: "2024-03-15", archived: true });
	const session = setRange(createSession([a, archived], dateFields), march);
	assert.deepEqual(paths(visibleTasks(session)), ["a"]);
});

test("parsedQuery / isQueryEmpty：解析关键字、标签、上下文", () => {
	const session = setQuery(createSession(tasks, dateFields), "#work @home alpha");
...
test("selectedTasks：返回可见且已勾选的任务", () => {
	const title = setAll(switchTab(createSession(tasks, dateFields), "title"), true);
	assert.deepEqual(paths(selectedTasks(title)), ["a", "b", "c"]);
	assert.deepEqual(paths(selectedTasks(toggle(title, "b"))), ["a", "c"]);
});

test("连续转移序列", () => {
	let session = createSession(tasks, dateFields);
